...
**- Pattern:** Removing the `PORT` variable in `dashboard.go` or `cmd/simple-dashboardd/main.go`.
...
**- Files Affected:** `dashboard.go` (formerly `main.go`), `cmd/simple-dashboardd/main.go`
...
**- Justification:** Large infrastructure changes and tooling standardization have been rejected (PRs #34, #36, #38, #41). The project prefers explicit build steps in CI.
